...
const headerConfig = [
  ...
  { title: "ف. داوطلبانه و ت. فردی", key: "voluntaryActivities", headerClass: "bg-pink-500 text-white", cellClass: "text-pink-700 font-semibold" }, // عنوان کوتاه شد
  { title: "فعالیت‌های شغلی", key: "jobActivities", headerClass: "bg-yellow-400 text-yellow-800", cellClass: "text-yellow-700 font-semibold" }, // رنگ هدر اصلاح شد
  ...
  { title: "رتبه در پایه", key: "rank", headerClass: "bg-gray-100 text-gray-700", cellClass: "text-indigo-700 font-bold" }, // عنوان به "رتبه در پایه" تغییر کرد
].reverse();

export default function StudentResultsPage({ Open }) { // نام کامپوننت را واضح‌تر کردم
  ...
  useEffect(() => {
    const fetchMyGradeResults = async (page = 1) => {
      ...
      try {
        ...
        const response = await fetchData(`users/my-grade-rankings?page=${page}&limit=15`, {
          headers: { authorization: `Bearer ${token}` }
        });

        console.log("--- StudentResultsPage - Data from API (my-grade-rankings) ---");
        console.log("Raw response from backend:", JSON.stringify(response, null, 2));

        if (response.success && Array.isArray(response.data)) {
          const formattedData = response.data.map(item => ({
              ...item,
              educationalActivities: formatNumberToPersian(item.educationalActivities),
              voluntaryActivities: formatNumberToPersian(item.voluntaryActivities),
          ...
          const foundUser = response.data.find(user => user.id === loggedInUserId);
  ...
  if (loading && resultsTableData.length === 0) {
    return <div className="flex justify-center items-center h-screen w-full"><p className="text-xl text-gray-600">در حال بارگذاری نتایج...</p></div>;
  }
  if (error && resultsTableData.length === 0) {
    return <div className="flex flex-col justify-center items-center h-screen w-full p-6 text-center">
        ...
        <button onClick={() => fetchMyGradeResults(1)} className="mt-4 bg-blue-500 text-white px-4 py-2 rounded">تلاش مجدد</button>
    </div>;
  }
      ...
      <div className={`p-6 md:p-8 transition-all duration-500 flex-col h-screen overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 ${!Open ? "w-[calc(100%-20%)]" : "w-[calc(100%-6%)]"}`}>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center h-auto sm:h-[5vh] mb-6">
            ...
            <div className="flex justify-center items-center gap-3 sm:gap-5">
                ...
                <h1 className="text-[#19A297] font-semibold text-base sm:text-lg">جدول امتیازات هم‌پایه‌ای‌ها</h1>
            </div>
        </div>

        {/* نمایش پیام‌های لودینگ و خطا در حین آپدیت جدول */}
        {loading && resultsTableData.length > 0 && <p className="text-center text-sm text-gray-500 py-2">در حال به‌روزرسانی جدول...</p>}
        {error && resultsTableData.length > 0 && !loading && <p className="text-center text-sm text-red-500 py-2">خطا: {error}</p>}
        ...
        {currentUserInfo && (
            <div className="mb-6 p-4 bg-indigo-50 rounded-lg shadow text-center">
                <p className="text-indigo-700">
                    ...
                    با امتیاز <span className="font-semibold">{formatNumberToPersian(currentUserInfo.score)}</span>،
                    رتبه <span className="font-semibold">{formatNumberToPersian(currentUserInfo.rank)}</span> را در پایه خود دارید.
                </p>
            </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1000px] text-sm">
              <thead className="text-xs uppercase sticky top-0 z-10"> {/* هدر چسبان */}
                <tr>
                  {headerConfig.map(header => (
                    <th key={header.key} scope="col" className={`px-3 py-3 font-semibold text-center ${header.headerClass}`}> {/* کاهش padding */}
                      {header.title}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-gray-200">
                {resultsTableData.length > 0 ? (
                  resultsTableData.map((row, idx) => (
                    <tr key={row.id || idx} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/60'} hover:bg-indigo-50/50 transition-colors text-xs`}> {/* کاهش سایز فونت */}
                      {headerConfig.map(header => (
                        <td key={`${row.id || idx}-${header.key}`} className={`px-3 py-2.5 whitespace-nowrap text-center ${header.cellClass}`}> {/* کاهش padding */}
                          {row[header.key]}
                        </td>
                      ))}
                    </tr>
                  ...
                  !loading && <tr><td colSpan={headerConfig.length} className="text-center py-10 text-gray-500">دانش‌آموزی در این پایه برای نمایش وجود ندارد.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {/* صفحه‌بندی */}
        {totalPages > 1 && (
            <div className="flex justify-center items-center space-x-1 sm:space-x-2 space-x-reverse mt-6 mb-8"> {/* کاهش فاصله */}
                <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1 || loading} className="px-3 py-1.5 text-xs sm:text-sm bg-gray-200 hover:bg-gray-300 rounded-md disabled:opacity-50"> قبلی </button>
                {/* منطق نمایش شماره صفحات با ... */}
                {[...Array(totalPages).keys()].map(num => {
                    const pageNum = num + 1;
                    if (totalPages <= 5 || (pageNum === 1 || pageNum === totalPages || Math.abs(pageNum - currentPage) <= 1) || (currentPage <= 2 && pageNum <= 3) || (currentPage >= totalPages - 1 && pageNum >= totalPages - 2) ) {
                        return ( <button key={pageNum} onClick={() => handlePageChange(pageNum)} disabled={loading} className={`px-3 py-1.5 text-xs sm:text-sm rounded-md ${currentPage === pageNum ? 'bg-[#19A297] text-white' : 'bg-white hover:bg-gray-100 border'}`}> {pageNum.toLocaleString('fa-IR')} </button> );
                    } else if ((currentPage > 3 && pageNum === 2) || (currentPage < totalPages - 2 && pageNum === totalPages - 1)) {
                        return <span key={pageNum} className="px-1 sm:px-2 py-1.5 text-xs sm:text-sm">...</span>;
                    }
                    return null;
                })}
                <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages || loading} className="px-3 py-1.5 text-xs sm:text-sm bg-gray-200 hover:bg-gray-300 rounded-md disabled:opacity-50"> بعدی </button>
                <span className="text-xs text-gray-600 hidden sm:inline"> صفحه {currentPage.toLocaleString('fa-IR')} از {totalPages.toLocaleString('fa-IR')} (کل: {totalResultsCount.toLocaleString('fa-IR')}) </span>
            </div>
        )}
